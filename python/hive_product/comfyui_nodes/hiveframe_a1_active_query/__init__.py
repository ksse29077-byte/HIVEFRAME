"""Repository-owned A1 Compound Eye regional active-query sampler node."""
...
RECEIPT_ENV = "HIVEFRAME_A1_RECEIPT"
_ACTIVE = False
...
class HIVEFRAMEH3ActiveQueryAttentionSampler(io.ComfyNode):
    """Run matched A1 CONTROL or conditional SELECTIVE sampling."""

    @classmethod
    def define_schema(cls):
        return io.Schema(
            node_id="HIVEFRAMEH3ActiveQueryAttentionSampler",
            category="hiveframe/a1",
            inputs=[
    ...
    def execute(
        ...
        global _ACTIVE
        if _ACTIVE:
            raise RuntimeError("A1 rejects concurrent sampler execution")
        if mode not in {CONTROL_MODE, SELECTIVE_MODE}:
            raise RuntimeError("A1 mode is unsupported")
        try:
            parsed = json.loads(admitted_blocks_json)
        except json.JSONDecodeError as error:
            raise RuntimeError("A1 admitted block list is malformed") from error
        if not isinstance(parsed, list) or any(not isinstance(value, int) for value in parsed):
            raise RuntimeError("A1 admitted block list must contain integers")
        admitted_blocks = tuple(parsed)
        if mode == CONTROL_MODE and admitted_blocks:
            raise RuntimeError("A1 CONTROL cannot carry admitted execution blocks")
        if mode == SELECTIVE_MODE and not admitted_blocks:
            raise RuntimeError("A1 SELECTIVE requires the frozen CONTROL block set")
        model_source = Path(h3_model.__file__).resolve()
        source_gate = inspect_installed_attention_source(
            model_source,
            model_source.parents[1] / "modules" / "attention.py",
        )
        if not source_gate.get("admitted") or source_gate.get("model_source_sha256") != MODEL_SOURCE_SHA256:
            raise RuntimeError("A1 installed H3 active-query source contract changed")
        ...
        controller = H3ActiveQueryAttentionController(
            mode=mode,
        ...
        payload: dict[str, Any] = {
            "schema_version": "a1.h3.active-query.callback.1",
            "mode": mode,
            "run_digest": run_digest,
            "workflow_revision_digest": workflow_revision_digest,
            "settings_digest": settings_digest,
...
class A1ActiveQueryExtension(ComfyExtension):
    async def get_node_list(self) -> list[type[io.ComfyNode]]:
        return [HIVEFRAMEH3ActiveQueryAttentionSampler]


async def comfy_entrypoint() -> A1ActiveQueryExtension:
    return A1ActiveQueryExtension()
